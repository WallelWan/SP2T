...
class SparsePooling(nn.Module):
    ...
    def __init__(
        ...
        if self.with_proj:
           ...
           self.proj_k = self.proj_o = nn.Identity()
        if residual:
            self.norm = nn.LayerNorm(num_ch)
        else:
            self.norm_loc = "none"
    ...
    def forward_torch(self, q, k, v, associations, bias):
        ...
        with record_function("Sparse Sim"):
            if bias is None:
                sim = torch.ones((N, H), device=device, dtype=torch.float32) # Pooling
            else:
                sim = bias # TRB-based Conv
        ...
        with record_function("Sparse Matmul"):
            # matmul
            k_expand = k_expand * weights.unsqueeze_(-1)
            k_expand = rearrange(k_expand, "n h d -> n (h d)")
            out = torch.zeros(Q, C, device=device, dtype=torch.float32)
            out = out.index_add_(0, dst_ids, k_expand)
