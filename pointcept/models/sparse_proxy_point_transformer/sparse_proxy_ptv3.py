...
from .sparse_attention import SparseAttention, SparsePooling
...
class Block(PointModule):
    def __init__(
        ...
        num_heads,
        patch_size=48,
        block_category="SparseAttention",
        mlp_ratio=4.0,
        qkv_bias=True,
        qk_scale=None,
        attn_drop=0.0,
        ...
        if self.with_spa:
            ...
            if block_category == "SparseAttention":
            ...
            elif block_category == "SparsePooling" or block_category == "SparseTRBConv":
                self.ca_down = SparsePooling(
                    channels, num_heads, attn_drop, drop_path,
                    residual=True, **ca_cfg
                )
                self.ca_up = SparsePooling(
                    channels, num_heads, attn_drop, drop_path,
                    residual=True, **ca_cfg
                )
            else:
                raise RuntimeError(f"Unknown block category: {block_category}")
    ...
    def forward(self, point: Point, info=None):
        if self.with_spa:
            ...
            with record_function(f"pt2px CA L{self.layer_idx}B{self.block_idx}"):
                if self.block_category != "SparsePooling":
                    tokens, weights0 = self.ca_down(q, k, v, asso, bias)
                else:
                    tokens, weights0 = self.ca_down(q, k, v, asso, None)
            if self.debug:
        ...
        if self.with_spa:
            ...
            with record_function(f"px2pt CA L{self.layer_idx}B{self.block_idx}"):
                if self.block_category != "SparsePooling":
                    ...
                    feat, weights1 = self.ca_up(q, k, v, asso, None)
            if self.debug:
...
class SparseProxyPTv3(PointModule):
    def __init__(
        ...
        enc_block_category_list=("SparseAttention", "SparseAttention", "SparseAttention", "SparseAttention", "SparseAttention"),
        dec_depths=(2, 2, 2, 2),
        dec_channels=(64, 64, 128, 256),
        dec_num_head=(4, 4, 8, 16),
        dec_patch_size=(48, 48, 48, 48),
        dec_block_category_list=("SparseAttention", "SparseAttention", "SparseAttention", "SparseAttention"),
        mlp_ratio=4,
        qkv_bias=True,
        qk_scale=None,
        attn_drop=0.0,
            ...
            for i in range(enc_depths[s]):
                enc.add(
                    Block(
                        ...
                        block_category=enc_block_category_list[s],
                        mlp_ratio=mlp_ratio,
                        qkv_bias=qkv_bias,
                        qk_scale=qk_scale,
                        attn_drop=attn_drop,
                ...
                for i in range(dec_depths[s]):
                    dec.add(
                        Block(
                            ...
                            block_category=dec_block_category_list[s],
                            mlp_ratio=mlp_ratio,
                            qkv_bias=qkv_bias,
                            qk_scale=qk_scale,
                            attn_drop=attn_drop,
